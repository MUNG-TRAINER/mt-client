...
export const trainerUserApi = {
  ...
  getTrainerUsers: async (): Promise<ITrainerUserListResponse[]> => {
    const res = await fetchWithAuth(`/api/trainer/users`, {
      method: "GET",
    });
    if (!res?.ok) {
  ...
  getUserDogs: async (userId: number): Promise<IDogResponse[]> => {
    const res = await fetchWithAuth(`/api/trainer/dogs/${userId}`, {
      method: "GET",
    });
    if (!res?.ok) {
  ...
  getDogStats: async (dogId: number): Promise<IDogStatsResponse> => {
    const res = await fetchWithAuth(`/api/trainer/user/dogs/${dogId}`, {
      method: "GET",
    });
    if (!res?.ok) {
